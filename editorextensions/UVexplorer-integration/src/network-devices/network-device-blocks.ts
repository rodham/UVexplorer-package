...
export class NetworkDeviceBlocks {
    ...
    private getCompany(device: Device) {
        ...
        if (typeof info_sets === 'object' && info_sets !== null) {
            if ('product_info' in info_sets && typeof info_sets.product_info === 'object' && info_sets.product_info !== null) {
    ...
    public convertDevicesToBlocks(devices: Device[]) {
        ...
        for(const device of devices) {
            ...
            const newBlock = new NetworkDeviceBlockModel(this.nextId as unknown as string, this.client, company, deviceType);
            this.incrementId();
