...
export class TopoMapComponent {
    ...
    private zoom(newZoom: number, location: Point) {
        if (newZoom > this.zoomMax) newZoom = this.zoomMax;

        if (newZoom < this.zoomMin) newZoom = this.zoomMin;
    ...
    private zoomToFit(): void {
        ...
        if (mapWidth > 0 && mapHeight > 0) {
            ...
            if (zoomValue > this.zoomMiddle) zoomValue = this.zoomMiddle;
    ...
    public showDeviceGroupOnMap(deviceGroupGuid: string): void {
        ...
        deviceGroupNode = this.map.deviceGroupNodes.find(
            (devGroupNode) => devGroupNode.deviceGroupGuid === deviceGroupGuid
        );
        if (deviceGroupNode) {
    ...
    private drawLabels(ctx: CanvasRenderingContext2D, labels: MouseOverLabel[], backgroundColor: string = null): void {
        ...
        labels.forEach((label) => {
            let found = combinedLabels.some((value: any) => {
                if (value.location.x == label.location.x && value.location.y == label.location.y) {
